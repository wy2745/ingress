...
func (this *summaryProvider) GetMetricsSources() []MetricsSource {
	...
	for _, node := range nodes {
		info, err := this.getNodeInfo2(node)
		if err != nil {
			glog.Errorf("%v", err)
			continue
		}
...
func (this *summaryProvider) getNodeInfo2(node *v1.Node) (NodeInfo, error) {
	...
	for _, addr := range node.Status.Addresses {
		if addr.Type == v1.NodeHostName && addr.Address != "" {
			info.HostName = addr.Address
		}
		if addr.Type == v1.NodeInternalIP && addr.Address != "" {
			info.IP = addr.Address
		}
		if addr.Type == v1.NodeLegacyHostIP && addr.Address != "" && info.IP == "" {
			info.IP = addr.Address
		}
	}
